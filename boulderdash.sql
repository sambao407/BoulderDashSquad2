...
--
-- Client :  127.0.0.1
-- Généré le :  Sam 24 Juin 2017 à 12:51
...
-- Procédures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `Call_Niveau1` ()  BEGIN
    SELECT * FROM Position, Element 
	WHERE FK_Element = ID_Element AND FK_Level =1;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Call_Niveau2` ()  NO SQL
BEGIN
    SELECT * FROM Position, Element 
	WHERE FK_Element = ID_Element AND FK_Level =2;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Call_Niveau3` ()  NO SQL
BEGIN
    SELECT * FROM Position, Element 
	WHERE FK_Element = ID_Element AND FK_Level =3;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Call_Niveau4` ()  NO SQL
BEGIN
    SELECT * FROM Position, Element 
	WHERE FK_Element = ID_Element AND FK_Level =4;

END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Call_Niveau5` ()  NO SQL
BEGIN
    SELECT * FROM Position, Element 
	WHERE FK_Element = ID_Element AND FK_Level =5;
